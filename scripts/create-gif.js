...
function startRecording() {
  ...
  document.getElementById("titleBox").innerHTML = "Capturando tu nuevo Guifo";
...
function uploadGif(gif) {
  ...
  fetch(
    "https://upload.giphy.com/v1/gifs?api_key=AVCzKbeHQ6uthBvk2ew0a7yq4uZ9i3Va",
    {
      method: "POST",
      body: gif
    }
    ...
    .then(data => {
      console.log(data);
      fetch(
        `https://api.giphy.com/v1/gifs/${data.data.id}?&api_key=AVCzKbeHQ6uthBvk2ew0a7yq4uZ9i3Va`
      )
        .then(response => {
          return response.json();
        })
